...
режима. «Повторить схему» проигрывает конечные этапы, а нажатие на любой этап
останавливает воспроизведение и показывает его пояснение. Это помогает
...
### Что происходит внутри KV-cache

Интерактивная схема Streaming разбивает один шаг генерации на шесть этапов:

1. Tokenizer один раз превращает prompt в `input_ids [1 × T]`.
2. Во время prefill каждый attention-слой рассчитывает и сохраняет Key и Value
   для всех `T` позиций. Старые Query не сохраняются: они уже использованы.
3. На следующем decode-шаге модель рассчитывает `Q_new`, `K_new` и `V_new`
   только для нового токена.
4. `Q_new` сравнивается со всеми Keys в cache, а attention-веса смешивают
   соответствующие Values в context vector.
5. `K_new` и `V_new` дописываются в cache, поэтому его длина становится `T+1`.
   Этот цикл повторяется для каждого следующего токена.
6. Готовый token id декодируется в текст. Только после этого streamer и SSE
   доставляют chunk в браузер.

KV-cache убирает повторный расчёт K и V для всей прошлой последовательности, но
занимает память. Её расход растёт с длиной контекста, числом слоёв и размером
batch. SSE к этой оптимизации не относится: он лишь доставляет уже готовые части
ответа раньше, чем завершится вся генерация.
