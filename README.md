...
все эксперименты без ручного `curl`. В каждом режиме блок «Как это работает»
раскрывает интерактивную вычислительную схему: её можно повторить целиком или
остановить на отдельном этапе — от HTTP payload и token ids до tensor batch,
GPU-вычисления и ответа. OpenAPI остаётся на `/docs`.
